...
func main() {
	...
	success.Printf("Started %d thread(s)", threads)
	wg.Wait() // Hachiko
}
...
func LikeThread(id int, wg *sync.WaitGroup) {
	...
	if err != nil {
		...
		req, err := http.NewRequest("GET", "http://194.58.115.48/add?id=" + photoID, nil)
		if err != nil {
			warning.Printf("Thread %d can`t create request", id)
		}

		resp, err := httpClient.Do(req)
		if err != nil {
			warning.Printf("Thread %d have a problem with request", id)
		}
